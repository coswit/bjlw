package com.bxlwt.www.bxlwt.city;
...
public class MySlideView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < CityPick.pinyinList.size(); i++) {
            String textView = CityPick.pinyinList.get(i);
            if (i == position - 1) {
                paint.setColor(getResources().getColor(R.color.error_color));
                selectTxt = CityPick.pinyinList.get(i);
            ...
            mTextHeight += mHeight / CityPick.pinyinList.size();
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                mTextHeight = mHeight / CityPick.pinyinList.size();
                position = y / (mHeight / (CityPick.pinyinList.size() + 1));
                invalidate();
                break;
            case MotionEvent.ACTION_MOVE:
                if (isSlide) {
                    backgroundColor = getResources().getColor(R.color.font_text);
                    mTextHeight = mHeight / CityPick.pinyinList.size();
                    position = y / (mHeight / CityPick.pinyinList.size() + 1) + 1;
                    invalidate();
                }
                break;
            case MotionEvent.ACTION_UP:
